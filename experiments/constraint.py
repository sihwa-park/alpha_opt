...
def propose_next(X_t, Y_t, Y_var, C_t, bounds, cfg) -> tuple[np.ndarray, float, dict]:
    ...
    models = ModelListGP(gp_obj, gp_con)
    fit_gpytorch_mll(SumMarginalLogLikelihood(models.likelihood, models))
    ...
    for j, ls in enumerate(
        models.models[0].covar_module.lengthscale.detach().flatten().tolist()
    ):
        gp_hparams[f"gp/lengthscale_{j}"] = ls

    if cfg["acquisition"] == "cei":
        acq = qLogNoisyExpectedImprovement(
            model=models,
            X_baseline=X_t,
            prune_baseline=cfg["prune_baseline"],
        ...
        return candidate.squeeze(0), float(acq_val.item()), gp_hparams
    else:
...
def maybe_save(
    trial_index, save_frequency, state_file, csv_file,
    param_names, X_np, Y_np, Y_var_np, results, wandb_run
):
    """Save state and upload wandb artifact every save_frequency evaluations."""
    if (trial_index + 1) % save_frequency != 0:
        return
    save_state(state_file, csv_file, param_names, X_np, Y_np, Y_var_np, results)
    try:
        artifact = wandb.Artifact(
            f"checkpoint_{trial_index + 1:06d}", type="checkpoint"
        )
        ...
        print(f"Warning: failed to upload checkpoint artifact: {e}")
...
def main():
    ...
    if cfg.get("initial_observations") is not None:
        ...
        C_t = torch.load(f"{path}/C_t.pt", map_location=device)
        ...
        torch.save(C_t, f"{save_path}/C_t.pt")
        try:
            ...
            artifact.add_file(f"{save_path}/C_t.pt")
            wandb_run.log_artifact(artifact)
    ...
    for i in range(trial_index, num_evals):
        t_gen = time.time()
        acq_val = None
        x, acq_val, gp_hparams = propose_next(X_t, Y_t, Y_var, C_t, bounds, cfg)
        ...
        if not result.failed:
            ...
            y_impute = float(Y_t[C_t < 0].mean()) if (C_t < 0).any() else 0.0
            y_var_impute = float(Y_t[C_t < 0].var()) if (C_t < 0).sum() > 1 else 1.0
            X_t = torch.cat([X_t, x.unsqueeze(0)])
            Y_t = torch.cat([Y_t, torch.tensor([y_impute], dtype=torch.double, device=device)])
            Y_var = torch.cat([Y_var, torch.tensor([y_var_impute], dtype=torch.double, device=device)])
            C_t = torch.cat([C_t, torch.tensor([1.0], dtype=torch.double, device=device)])
            result.loss = y_impute

        wandb.log(
            {
                "iteration": i,
                "loss": result.loss,
                "constraint": C_t[-1].item(),
        ...
        results.append(result)
        # maybe_save(
        #     i, save_frequency, state_file, csv_file,
        #     param_names, X_np, results, wandb_run,
        # )
    ...
    # --- Best result ---
    # successful = [(i, r) for i, r in enumerate(results) if not r.failed]
    # if successful:
    #     best_idx, best_result = min(successful, key=lambda t: t[1].loss)
    #     print(f"Best trial: index={best_idx}, loss={best_result.loss:.6e}, "
    #           f"worst_dmerc={best_result.worst_dmerc:.6e}")
    #     print(f"Best x: {X_np[best_idx]}")

    #     wandb_run.summary["best_index"] = best_idx
    #     wandb_run.summary["best_loss"] = best_result.loss
    #     wandb_run.summary["best_worst_dmerc"] = best_result.worst_dmerc
    #     for name, val in zip(param_names, X_np[best_idx]):
    #         wandb_run.summary[f"best_x/{name}"] = float(val)

    #     # Re-evaluate to leave vmec/wout in optimised state.
    #     evaluate(X_np[best_idx], objective, vmec, cfg, 999999, run_name)
    #     vmec.run()
    #     vmec.wout.save("wout_optimized.nc")
    #     with open("input.optimized", "w") as f:
    #         f.write(vmec.indata.to_json(indent=2))
    #     print("Saved wout_optimized.nc and input.optimized.")

    #     try:
    #         artifact = wandb.Artifact("optimization_results", type="results")
    #         for path in [state_file, csv_file, "wout_optimized.nc", "input.optimized"]:
    #             if os.path.exists(path):
    #                 artifact.add_file(path)
    #         wandb_run.log_artifact(artifact)
    #         print("Uploaded optimization_results artifact to wandb.")
    #     except Exception as e:
    #         print(f"Warning: failed to upload final artifact: {e}")
    # else:
